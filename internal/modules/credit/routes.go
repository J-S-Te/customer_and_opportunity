...
func RegisterRoutes(api *gin.RouterGroup, h *Handler) {
	...
	api.GET("/credit/applications/pending", middleware.RequirePermissionOrRoles("customer.credit.approve", "sales_director", "crm_super_admin"), h.Pending)
	api.POST("/credit/applications/:applicationID/approve", middleware.RequirePermissionOrRoles("customer.credit.approve", "sales_director", "crm_super_admin"), h.Approve)
	api.POST("/credit/applications/:applicationID/reject", middleware.RequirePermissionOrRoles("customer.credit.approve", "sales_director", "crm_super_admin"), h.Reject)
