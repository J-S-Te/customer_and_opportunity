...
> 2026-09-07 文件策略更新：ClamAV 病毒引擎已从源码、配置和可选部署服务移除；保留静态文件校验、权限、对象一致性和文件网关。历史扫描状态保持兼容，但不代表病毒检测。本文较早的病毒扫描要求以 [当前文件校验策略](docs/file-validation-policy.md) 为准；本地代码修改尚未发布生产镜像。
