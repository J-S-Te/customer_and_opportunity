...
for c in "$customer_mysql" "$portal_mysql" "$customer_api" "$portal_api" "$platform_mysql"; do
  docker inspect "$c" >/dev/null 2>&1 || {
    echo "找不到容器：$c（可设置 CUSTOMER_MYSQL_CONTAINER 等变量）" >&2
    exit 1
  }
done
...
[[ -n "$customer_dsn" && -n "$portal_dsn" ]] || {
  echo "无法从容器读取 MySQL DSN" >&2
  exit 1
}
...
[[ -n "$platform_root_password" ]] || {
  echo "无法从平台 MySQL 容器读取 MYSQL_ROOT_PASSWORD" >&2
  exit 1
}

platform_owner_row() {
  docker exec "$platform_mysql" sh -c \
    "MYSQL_PWD='$platform_root_password' mysql \
      --default-character-set=utf8mb4 -h127.0.0.1 -uroot \
      -D basic_platform -Nse \"$1\""
}
...
resolve_owner() {
  ...
  platform_owner_row "
    ...
    WHERE u.tenant_id='01J00000000000000000000000'
      AND u.status='ACTIVE'
      AND u.deleted_at IS NULL
      AND u.employment_status <> 'EXTERNAL_CUSTOMER'
      AND EXISTS (
        ...
        WHERE b.tenant_id=u.tenant_id
          AND a.code='customer_and_opportunity'
          AND b.status='ACTIVE'
          AND r.status='ACTIVE'
          AND r.role_type <> 'COMPATIBILITY'
          AND (b.valid_from IS NULL OR b.valid_from <= NOW())
          AND (b.valid_until IS NULL OR b.valid_until > NOW())
          AND (
            (b.scope_type='TENANT' AND b.scope_id='')
            OR (
              b.scope_type='ENVIRONMENT'
              AND b.scope_id IN (
                SELECT e.id
                FROM platform_application_environment e
                WHERE e.application_id=a.id AND e.environment='dev' AND e.status='ACTIVE'
              )
            )
          )
          AND (
            (b.subject_type='USER' AND b.subject_id=u.id)
            OR (m.inherit_authorization=1 AND b.subject_type='ORG_UNIT' AND b.subject_id=m.org_unit_id)
            OR (
              m.inherit_authorization=1
              AND b.subject_type='POSITION'
              AND b.subject_id=m.position_id
              AND EXISTS (
                SELECT 1
                FROM iam_position p
                WHERE p.id=m.position_id AND p.org_unit_id=m.org_unit_id AND p.status='ACTIVE'
              )
            )
          )
      )
