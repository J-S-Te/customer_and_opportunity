...
迁移必须由目标环境的 migration 入口执行，不允许通过直接向业务表插入模拟数据绕过合同同步、应收确认、权限目录或状态校验。涉及 Portal 邀请、附件扫描、报告下载和项目访问绑定的迁移，执行后应按对应 `DEVELOPMENT.md` 验收接口、Worker 心跳和审计记录。

当前测试数据应通过 API/事件入口生成，并使用独立租户和测试账号；生产发布使用不可变镜像和受控 migration，失败时保留日志与 `request_id`，不得手工把状态改成成功。
